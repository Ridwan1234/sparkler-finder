  ...
  IF NEW.status = 'approved' AND (OLD.status IS NULL OR OLD.status <> 'approved') THEN
    ...
    IF previous_approved_count = 0 THEN
      ...
      IF bonus_percent IS NOT NULL AND bonus_percent > 0 THEN
        ...
        VALUES (NEW.user_id, bonus_amount, 'bonus', 'First deposit bonus (' || bonus_percent || '%)', NEW.id);
      END IF;

      SELECT referred_by, COALESCE(full_name, 'A user') INTO referrer_code, referred_user_name
        ...
        IF referrer_user_id IS NOT NULL THEN
          ...
          IF referral_percent IS NOT NULL AND referral_percent > 0 THEN
            ...
            VALUES (
              ...
              referral_amount,
              'referral_bonus',
              'Referral bonus (' || referral_percent || '%) from ' || referred_user_name,
              NEW.id
            );
          END IF;
        END IF;
